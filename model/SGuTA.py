...
class TransFrame(nn.Module):
    def __init__(
        ...
        self.to_q = nn.Linear(dim* n_inputs, dim_head * heads * n_inputs, bias=False)
        self.to_k = nn.Linear(dim* n_inputs, dim_head * heads * n_inputs, bias=False)
        self.to_v = nn.Linear(dim* n_inputs, dim_head * heads * n_inputs, bias=False)
        self.rescale = nn.Parameter(torch.ones(heads, 1, 1))
        self.proj = nn.Linear(dim_head * heads * n_inputs, dim * n_inputs, bias=True)
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                PreNorm(dim, TransFrame(dim=dim, dim_head=dim_head, heads=heads, n_inputs=n_inputs)),
...
class TransUnet(nn.Module):# 论文Figure 2.(b) SST
    def __init__(self, in_channels=3, out_channels=3, emb_dim=32, stage=2, patch_size=(1,4,4), num_blocks=[2,4,4], n_inputs=8):
        ...
        for i in range(stage):
            self.encoder_layers.append(nn.ModuleList([
                FAB(dim=dim_stage, num_blocks=num_blocks[i], dim_head=emb_dim, heads=dim_stage // emb_dim, n_inputs=n_inputs), # SAB 模块
                nn.Conv3d(dim_stage, dim_stage * 2, 4, 2, 1, bias=False), # DownSample 每次Channel*2,H/2,W/2
            ])) 
            dim_stage *= 2
            n_inputs //= 2
        # Bottleneck
        self.bottleneck = FAB(dim=dim_stage, dim_head=emb_dim, heads=dim_stage // emb_dim, num_blocks=num_blocks[-1], n_inputs=n_inputs)
        n_inputs *= 2
        # Decoder
        ...
        for i in range(stage):
            self.decoder_layers.append(nn.ModuleList([
                ...
                FAB(dim=dim_stage // 2, num_blocks=num_blocks[stage - 1 - i], dim_head=emb_dim, heads=(dim_stage // 2) // emb_dim, n_inputs = n_inputs ),
            ]))
            dim_stage //= 2
            n_inputs *= 2
        # Output projection
...
class SGuTA(nn.Module): # Figure2 a)
    def __init__(self, in_channels=3 , n_inputs=8, n_outputs=1, n_feat=64, patch_size=(1,4,4), stage=1, **kwargs):
        super(SGuTA, self).__init__()
        out_channels = in_channels*n_outputs
        modules_body = [TransUnet(in_channels, out_channels, emb_dim=n_feat, stage=2, patch_size=patch_size, num_blocks=[1,1,1], n_inputs=n_inputs) for _ in range(stage)]
...
if __name__ == "__main__":
    model = SGuTA(in_channels=3 , n_inputs=8, n_outputs=1, n_feat=64, patch_size=(1,4,4), stage=1).cuda()
