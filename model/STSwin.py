...
def cube_partition(x, cube_size):
    ...
    """
    B, D, H, W, C = x.shape
    b_d, b_h, b_w =D, cube_size[1], cube_size[2]
    # if b_d == 0:
    #     b_d = D
...
def compute_attn_mask(D, H, W, cube_size=(2,8,8)):
    b_d, b_h, b_w = D, cube_size[1], cube_size[2]  # block depth/height/width    
...
class TransCube(nn.Module):
    ...
    def forward(self, x, move_block):
        ...
        c_d, c_h, c_w = d, cube_size[1], cube_size[2]       
        if move_block:
...
class STSwin(nn.Module): # Figure2 a)
    def __init__(self, in_channels=3, out_channels=3, n_feat=64, patch_size=(1,4,4), cube_size=(2,4,4), stage=2):
        super(STSwin, self).__init__()
        # self.conv_in = nn.Conv3d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False) # 3*3卷积
...
if __name__ == "__main__":
    model = STSwin(in_channels=3, out_channels=3, n_feat=64, patch_size=(1,4,4), cube_size=(2,4,4), stage=2).cuda()    
