    ...
    for d in d_slices:
        for h in h_slices:
            for w in w_slices:
                mask[:, d, h, w, :] = cnt
                cnt += 1
    mask_cubes = cube_partition(mask, cube_size)  # nW, window_size, window_size, 1
...
class TransCube(nn.Module):
    ...
    def forward(self, x, move_cube):
        ...
        if move_cube:
            ...
            n_B = mask.shape[0]
            N = c_d* c_h* c_w
            attn = attn.view(b, n_B, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)
...
if __name__ == "__main__":
    model = SCubA_Ntd(in_channels=3, n_outputs=1, n_feat=36, patch_size=(1,3,3), cube_size=(2,8,8), stage=1).cuda()
