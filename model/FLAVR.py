...
class FLAVR(nn.Module):
    def __init__(self, n_inputs, n_outputs, in_channels, block='unet_18', batchnorm=False , joinType="concat" , upmode="transpose", **kwargs):
        super().__init__()
        ...
        self.encoder = getattr(unet_3D , block)(pretrained=False , bn=batchnorm, in_channels=in_channels)            
    ...
    def forward(self, images):
        ...
        b, c, d_inp, h_inp, w_inp = images.shape
        db, hb, wb = self.n_inputs, 8, 8 
        pad_h = (hb - h_inp % hb) % hb
        pad_w = (wb - w_inp % wb) % wb
        pad_d = (db - d_inp % db) % db
        images = F.pad(images, [pad_w//2, pad_w//2, pad_h//2, pad_h//2, pad_d//2,pad_d//2], mode='constant')#(left_pad, right_pad, top_pad, bottom_pad, front_pad, back_pad)
        ...
        out = torch.split(out, dim=1, split_size_or_sections=self.in_channels)
...
if __name__ == '__main__':
    model = FLAVR(n_inputs=4, n_outputs=1, in_channels=3)
    print('# of parameters: %d' % sum(p.numel() for p in model.parameters()))
    
    b,c,d,h,w = 2, 3, 4, 128,128
